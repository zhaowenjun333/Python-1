...
//     await sleep(ms);
// }
function insert(arrayList,index,value){
    arrayList.splice(index,0,value);
    return arrayList;
}
function ArraySort(arrList) {
...
function move_data(d, n) {
    var traceData = [],
    x = 0,
    y_lst = [0, 1],
    x_lst = [x, d],
    step = Math.floor(d/50),
    index = 0,
    t = datetime(),
    begin_time = datetime(),
    ave_d = Math.round(d/2)
    let middle_x_lst;
    let ccf;
    t = t-begin_time
    while (traceData.length < 50) {
        ccf = [x_lst[index], choice.sample(y_lst), t]
        console.log(ccf)
        traceData.push(i(n, ccf + ""));
        var ins = RandomNum(x_lst.at(index), step + x_lst.at(index))
        if (Math.floor((x_lst.at(-1) - ins) / step) > (47 - index)) {
            ins += 1
        } else if (Math.floor((x_lst.at(-1) - ins) / step) === (47 - index)) {
            ins += 0
        } else {
            ins = RandomNum(x_lst.at(0), x_lst.at(-1))
        }
        if (index > 12) {
            middle_x_lst = Math.ceil((x_lst.at(-1) - x_lst.at(12)))
            if (middle_x_lst > ave_d) {
                ins += Math.ceil((middle_x_lst - ave_d) / 37)
            }
        }
        if ((x_lst.indexOf(ins) === -1) && (x_lst[0] < ins < d) && (x_lst.length < 50)) {
            insert(x_lst, index + 1, ins)
            ArraySort(x_lst)
        }
        t += RandomNum(70, 80)
        index += 1
    }
    let cc1 = sample(traceData, 50),
        cc2 = B(i(n, 2 + "," + traceData.length))
...
console.log(move_data(75, '5dd97da6c3f04d78b1e0e9c8e65734ae'))
