...
class B64ImgSpider:
    def __init__(self):
        self.api_url = 'https://match.yuanrenxue.com/api/match/4'
        ...
        self.sessionid = 'g49nwqvz4j8w72mm5scradwe2wkbnaqd'
        self.cookies = {
            "sessionid": self.sessionid,
            'Hm_lvt_9bcbda9cbf86757998a2339a0437208e': '1684192630,1684192871,1684317862,1684472531',
            'Hm_lvt_c99546cf032aaa5a679230de9a95c7db': '1684192605,1684192866,1684317806,1684472524',
            'Hm_lpvt_9bcbda9cbf86757998a2339a0437208e': '1684475002',
            'Hm_lpvt_c99546cf032aaa5a679230de9a95c7db': '1684475011',
            'no-alert3': 'true',
            'qpfccr': 'true',
            'tk': '-915992158495534545'
        }
    ...
    def svg_img(self, md5_str, info_str):
        data_10 = re.findall(f'<td>(.*?)</td>', info_str)
        ong_page_sum = 0
        for data_1 in data_10:
            info_list = re.findall(
                r'<img src="data:image/png;base64,(.*?)" class="img_number (.*?)" style="left:(.*?)px">', data_1, re.S)
            num_list = []  # 实际数据
            svg_list = []  # 偏移量
            for info in info_list:
                if md5_str != info[1]:
                    # 实际数据
                    num = self.dict0[info[0]]
                    num_list.append(num)
                    # 偏移量
            ...
            # print(svg_list)
            new_lst = []
            for index in range(len(svg_list)):
                t = num_list[index]
                zuhe = [index, t]
                zuhe[0] += svg_list[index]
                new_lst.append(zuhe)
            new_lst.sort(key=lambda x: x[0])
            number = int(''.join([str(i[1]) for i in new_lst]))
    ...
    def run(self):
        ...
        for page in range(1, 6):
            ...
            content = resp.json()
            k, v = content['key'], content['value']
            md5_str = self.enstr(k, v)
            info = content['info']
            self.svg_img(md5_str, info)
        print(sum(self.num))
