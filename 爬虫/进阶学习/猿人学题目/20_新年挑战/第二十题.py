...
class NumberSpider:
    ...
    def get_sign(self, page, tim):
        s = f'{page}|{tim}D#uqGdcw41pWeNXm'
        sign = md5(s.encode()).hexdigest()
        return sign

    def deal_sum(self, params):
        tim = execjs.compile("function get_t(){return Date.parse(new Date())}").call('get_t')
        sign = self.get_sign(params['page'], tim)
        params['sign'] = sign
        params['t'] = tim
        print(params)
        if params['page'] > 3:
