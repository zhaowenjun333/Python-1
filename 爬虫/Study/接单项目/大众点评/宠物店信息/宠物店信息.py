...
if __name__ == '__main__':
    ...
    with open('address.txt', 'r', encoding='utf-8') as address_txt:
        address_dict = eval(address_txt.readline())
    with open('num.txt', 'r', encoding='utf-8') as num_txt:
        num_dict = eval(num_txt.readline())
    with open('tagName.txt', 'r', encoding='utf-8') as tagName_txt:
    ...
    writer = csv.writer(f)
    header = ('宠物店名称', '宠物店类型', '评分', '评价数量', '费用', '地址', '电话号码')
