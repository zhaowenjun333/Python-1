...
class BeiKeTest(threading.Thread):
    headers = [
        # Chrome
        {
...
class BeiKeProducer(threading.Thread):
    ...
    def parse_page(self, url):
        ...
        resp.close()
        html_element = etree.HTML(html)
        # ros = html_element.xpath('//div[@class="row"]/h1/text()')[0]
        # 上锁
        # mutex.acquire()
        # if self.driver.page_source.find('//div[@class="row"]/h1/text()'):
        #     self.man_machine(url)
        #     time.sleep(3)
        # mutex.release()  # 解锁
        buildings = html_element.xpath('//ul[@class="resblock-list-wrapper"]/li')
        if buildings:
...
class BeiKeConsumer(threading.Thread):
    ...
    def saveData(self, lst):
        ...
        with open('./CSV/贝壳找房_杭州18.csv', 'w', encoding='utf-8-sig', newline='') as f:
...
if __name__ == '__main__':
    ...
    ur = 'https://hz.fang.ke.com/loupan'
    ...
    t_lst = []
    for i in range(25):
        te = BeiKeTest(test_queue)
        te.start()
        t_lst.append(te)
    for t in t_lst:
        t.join()
    # 实例化人机验证
    man_machine = BeiKeManMachine()
    man_machine.man_machine(ur)
